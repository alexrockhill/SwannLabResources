"""
https://raw.githubusercontent.com/voytekresearch/Cole_2017/master/shape.py
...
def ex_sharp(x, Es, widthS, ampPC=0, Fs=1000, fosc=(13, 30), method='diff'):
    ...
    Es : array-like 1d
        time points of oscillatory peaks or troughs
    widthS : int
        Number of samples in each direction around extrema
        to use for sharpness estimation
...
def rdsr(Rsteep, Dsteep):
    return np.max((np.mean(Rsteep) / np.mean(Dsteep),
                   np.mean(Dsteep) / np.mean(Rsteep)))
